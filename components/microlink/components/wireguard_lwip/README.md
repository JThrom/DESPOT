...
In DumbESPty, this WireGuard component is unchanged functionally. Keyboard input
is provided via BLE HID (`main/ble_hid_host.cpp`). Wired USB keyboard input is
not supported on the CrowPanel Advanced 9" (its USB Type-C port is wired as a
device port); the `usb_hid_host.cpp` module remains in the build but is
non-functional on that board.
