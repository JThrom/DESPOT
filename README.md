```text
 8888888b.   8888888888   .d8888b.   8888888b.    .d88888b.   88888888888
 888   Y88b  888         d88P  Y88b  888   Y88b  d88P" "Y88b      888
 888    888  888         Y88b.       888    888  888     888      888
 888    888  8888888      "Y888b.    888   d88P  888     888      888
 888    888  888             "Y88b.  8888888P"   888     888      888
...
**DESPOT** — **D**umb **E**sp32 **S**ecure **P**ortable **O**pen **T**erminal.

Name meaning, letter by letter:

- **D — Dumb**: a classic dumb-terminal interaction model; the device renders a
  remote session rather than running local applications.
- **E — Esp32**: built on the Espressif ESP32 MCU family (primary target:
  ESP32-P4, with an ESP32-C6 companion for Wi-Fi 6 + BLE).
- **S — Secure**: SSHv2 transport with host-key trust (TOFU pinning),
  publickey/keyboard-interactive/password auth, and an AES-encrypted key vault.
- **P — Portable**: a self-contained, battery-capable touch device you can carry.
- **O — Open**: open-source firmware (GNU/GPL v2).
- **T — Terminal**: a VT100/xterm-style, 256-color terminal emulator.

## Overview

DESPOT is a portable, color dumb-terminal style system built for ESP32-P4 touch display boards. The primary/default target is the Elecrow CrowPanel Advanced 9-inch ESP32-P4 HMI (1024x600 IPS touch, Wi-Fi 6); the Waveshare ESP32-P4-WIFI6-Touch-LCD-7B is a secondary supported board. It combines:
...
- Default target is the CrowPanel Advanced 9" (1024x600); the Waveshare 7"
  board is selectable via a build overlay (see Build below).
...
- Wired USB keyboard input is NOT supported on the CrowPanel Advanced 9". The
  board's USB2.0 Type-C port (J16) is wired as a USB *device* port (VBUS is an
  input, diode-fed into the system 5V rail; there is no VBUS output switch and
  no host `DRVVBUS` routing), so it cannot power or enumerate a downstream
  keyboard. Elecrow's own USB example runs this port in TinyUSB *device* mode.
  Use a BLE keyboard instead.
...
- The USB HID host module and its boot-keyboard keymap remain in the build, but
  wired USB keyboards do not work on the CrowPanel Advanced 9". The board's
  USB2.0 Type-C port (J16) is wired as a USB *device* port: `+VBUS2` is diode-fed
  into the system 5V rail (input only), there is no VBUS output load switch, and
  no host `DRVVBUS` routing. The port therefore cannot supply VBUS to or
  enumerate a downstream keyboard, regardless of firmware. Elecrow's own USB
  example (`Lesson06-USB2.0`) drives this port in TinyUSB *device* mode.
- ESP32-P4 chip revision is not a factor: rev 1.x fully supports USB host; the
  limitation is the board's port wiring, not the silicon.
- Use a BLE keyboard for wired-free input.
...
DEVICE section:
- Product and MCU strings selected by build target and board
  (CrowPanel Advanced 9" primary / Waveshare P4 7" / legacy ESP32-S3)
- DESPOT version: runtime app metadata (currently `1.0.0`)
- Author: `Jason Throm`
- GitHub: `https://github.com/JThrom/DESPOT`
- License: `GNU/GPL v2`
- Display details: panel label (e.g. `9" IPS MIPI panel`), live terminal grid
  and cell size values
- I/O expander: `none (CrowPanel)` on the primary board;
  `CH422G (I2C init path enabled)` on the Waveshare board
...
The default build targets the primary board (CrowPanel Advanced 9"):
...
### Building for the secondary Waveshare 7" board

Layer the Waveshare overlay on top of the base defaults:
...
You can also switch boards interactively via
`idf.py menuconfig` -> `DESPOT Board Selection` -> `Target hardware board`.

ESP32-P4 flash-size note (important):

- The CrowPanel Advanced 9" (ESP32-P4NRW32) has `16MB` flash; P4 defaults pin
  the image header to `16MB` in `sdkconfig.defaults.esp32p4`
  (`CONFIG_ESPTOOLPY_FLASHSIZE_16MB=y`).
- The secondary Waveshare board has `32MB` flash; the Waveshare overlay
  (`sdkconfig.defaults.waveshare_p4_7`) sets `CONFIG_ESPTOOLPY_FLASHSIZE_32MB=y`.
- Keep the header matched to the board. A header smaller than the physical
  flash logs `Detected size(...) larger than the size in the binary image
  header(...)` and consumed multiple failed bring-up attempts previously.
...
## Using DESPOT Inside tmux (truecolor / COLORTERM)
