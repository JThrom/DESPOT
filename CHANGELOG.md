...
All notable changes to DESPOT are documented in this file.

## [Unreleased]

### Removed / Corrected (Input) - 2026-07
- Documented that wired USB keyboard input is **not supported on the CrowPanel
  Advanced 9"** and removed it from the advertised feature set. Root cause is
  hardware, not firmware: the board's USB2.0 Type-C port (J16) is wired as a USB
  *device* port. `+VBUS2` is Schottky-diode-fed into the system 5V rail (VBUS
  input only) with no VBUS output switch and no host `DRVVBUS` routing, so the
  port cannot power or enumerate a downstream keyboard. J16 D+/D- reach P4 pads
  49/50 (HS/UTMI controller [0], the USB host library default), but the
  surrounding circuit is device-oriented; Elecrow's own USB example
  (`Lesson06-USB2.0`) drives this port in TinyUSB device mode.
- Clarified this is **not** a chip-revision issue: ESP32-P4 rev 1.x fully
  supports USB host; the rev-gated `CONFIG_ESP32P4_SELECTS_REV_LESS_V3` only
  affects D+/D- pulldown ownership on rev >= 3 and is correct for this silicon.
- `usb_hid_host.cpp` and its boot-keyboard keymap remain in the build (and unit
  tests) but are non-functional on this board. Use a BLE keyboard.
- `about` DEVICE section keyboard line now reads `BLE HID` (was
  `BLE HID + USB OTG HID host`).

### Added (Board Support) - 2026-07
- Added the Elecrow CrowPanel Advanced 9" ESP32-P4 HMI (1024x600 IPS touch,
  Wi-Fi 6) as a supported target and made it the **primary/default board**:
  - new Kconfig board selector `DESPOT Board Selection` (default CrowPanel),
  - new display backend `main/crowpanel_display_p4.cpp` (EK79007 MIPI-DSI,
    no CH422G, GT911 touch on I2C SDA45/SCL46 RST40/INT42, backlight PWM on
    GPIO31 active-high, native orientation),
  - P4 defaults (`sdkconfig.defaults.esp32p4`) now target CrowPanel (16MB
    flash, default brownout level).
- The Waveshare ESP32-P4-WIFI6-Touch-LCD-7B is now the **secondary** board,
  built via an overlay:
  `-DSDKCONFIG_DEFAULTS="sdkconfig.defaults;sdkconfig.defaults.esp32p4;sdkconfig.defaults.waveshare_p4_7"`
  (32MB flash, lowered brownout level for its USB->battery power-path dip).
- The `about` command's DEVICE section now reflects the selected board
  (product string, panel label, and IO-expander line).

### Changed (Shell) - 2026-07
- Command-line tokenizer is now quote-aware (`"..."` / `'...'`, backslash
  escapes) so arguments with spaces (e.g. a Wi-Fi SSID like `My Network`)
  parse as a single token. `wifi connect`/`wifi forget` also reassemble an
  unquoted multi-word SSID.
