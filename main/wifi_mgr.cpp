...
// Join argv[from .. argc-1] with single spaces into `dst`, then trim trailing
// whitespace. This lets an unquoted SSID with spaces (e.g. "My Network",
// tokenized into several argv entries) be reassembled into one SSID. A quoted
// SSID arrives as a single argv entry and passes through unchanged.
static void join_ssid_args(int argc, char **argv, int from, char *dst, size_t dst_len) {
    if (!dst || dst_len == 0) return;
    dst[0] = '\0';
    if (from >= argc) return;

    size_t pos = 0;
    for (int i = from; i < argc && pos < dst_len - 1; i++) {
        if (i > from && pos < dst_len - 1) {
            dst[pos++] = ' ';
        }
        pos += snprintf(dst + pos, dst_len - pos, "%s", argv[i]);
        if (pos >= dst_len - 1) {
            pos = dst_len - 1;
            break;
        }
    ...
    // Trim trailing whitespace (leading handled by tokenizer already).
    while (pos > 0 && isspace((unsigned char)dst[pos - 1])) {
        dst[--pos] = '\0';
    }
}
...
bool wifi_mgr_get_rssi(int *rssi_dbm) {
    if (!rssi_dbm || !connected) return false;
    wifi_ap_record_t ap = {};
...
void wifi_mgr_get_status(char *buf, size_t len) {
    if (connected) {
        ...
        if (wifi_mgr_get_rssi(&rssi)) {
            ...
            if (used < len) {
                snprintf(buf + used, len - used, " (RSSI %d dBm)", rssi);
            }
        }
    } else {
...
void cmd_wifi(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "scan") == 0) {
        ...
        if (argc < 3) {
            ...
            shell_print("\r\n  tip: SSIDs with spaces work unquoted (wifi connect My Network)\r\n");
            shell_print("\r\n       or quoted (wifi connect \"My Network\")\r\n");
            shell_print("\r\n  tip: use TAB for saved SSID completion\r\n");
            return;
        }

        char ssid[33];
        join_ssid_args(argc, argv, 2, ssid, sizeof(ssid));
        if (!ssid[0]) {
            shell_print("\r\n  usage: wifi connect <ssid>\r\n");
            return;
        }
