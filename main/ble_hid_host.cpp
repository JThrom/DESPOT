...
// ESP-Hosted: the BLE controller lives on the ESP32-C6 co-processor and must be
// explicitly initialized + enabled over the hosted transport before NimBLE
// sends any HCI. Without these the C6 controller never starts and every HCI
// command times out (BLE_HS_ETIMEOUT_HCI).
...
esp_err_t ble_hid_host_init(void) {
...
#if CONFIG_IDF_TARGET_ESP32P4
    // Bring up the BLE controller on the C6 co-processor over ESP-Hosted before
    // starting the NimBLE host. The transport is already up from wifi_mgr_init()
    // (esp_hosted). These are no-ops of last resort on other targets.
    esp_err_t bt_ret = esp_hosted_bt_controller_init();
    if (bt_ret != ESP_OK) {
        ESP_LOGE(TAG, "esp_hosted_bt_controller_init failed: %s", esp_err_to_name(bt_ret));
    }
    bt_ret = esp_hosted_bt_controller_enable();
    if (bt_ret != ESP_OK) {
        ESP_LOGE(TAG, "esp_hosted_bt_controller_enable failed: %s", esp_err_to_name(bt_ret));
    }
#endif
    ...
    ble_svc_gap_init();
    ble_svc_gatt_init();
    ble_svc_gap_device_name_set("DESPOT");
