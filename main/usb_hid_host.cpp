...
static void usb_lib_task(void *arg) {
    ...
    // ESP32-P4 exposes two USB-DWC controllers:
    //   [0] High-Speed, UTMI PHY. The dedicated analog DP/DM pads (chip pins
    //       49/50) belong to this path (shared with USB-Serial-JTAG).
    //   [1] Full-Speed, internal FSLS PHY on GPIO27(DP)/GPIO26(DM).
    // On the CrowPanel Advanced 9" the host Type-C port (J16) is wired to the
    // dedicated DP/DM pads (49/50) -> High-Speed controller [0]. GPIO26/27 (the
    // FS controller's pins) are used on this board for I2S_SDIN2 / P4_TXD2 and
    // are NOT routed to J16. So the host MUST bind controller [0], which is the
    // library default (peripheral_map == 0 selects HS on HS-capable targets).
