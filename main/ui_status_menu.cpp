...
static i2c_master_bus_handle_t get_touch_i2c_bus(void) {
    ...
    static const i2c_pin_pair candidates[] = {
        ...
        // Elecrow CrowPanel Advanced 9": GT911 on I2C SDA45/SCL46. Probe first.
...
// Battery color by remaining charge: green normally, yellow below 30%, red at
// or below 10%. Green (0x2ECC71) / yellow (0xF1C40F) / red (0xE74C3C).
static lv_color_t battery_level_color(uint8_t percent) {
    if (percent <= 10) return lv_color_hex(0xE74C3C);
    if (percent < 30) return lv_color_hex(0xF1C40F);
    return lv_color_hex(0x2ECC71);
}

// Pick a battery glyph that roughly reflects the charge level.
static const char *battery_level_glyph(uint8_t percent) {
...
static void brightness_slider_event_cb(lv_event_t *e) {
    ...
    // Persist only when the drag finishes (RELEASED), not on every intermediate
    // VALUE_CHANGED, to avoid hammering NVS/flash while sliding.
    if (code == LV_EVENT_RELEASED) {
        brightness_save(percent);
    }
}
...
static esp_err_t touch_init_internal(void) {
    ...
    for (size_t i = 0; i < sizeof(probe_addrs) / sizeof(probe_addrs[0]); i++) {
...
#if CONFIG_DESPOT_BOARD_CROWPANEL_ADV_9
        // Elecrow CrowPanel Advanced 9": GT911 reset GPIO40, INT GPIO42.
        touch_cfg.rst_gpio_num = GPIO_NUM_40;
        touch_cfg.int_gpio_num = GPIO_NUM_42;
#else
        ...
        // CrowPanel display driver now uses native orientation (no rotation),
        // so touch needs no mirroring to match.
...
esp_err_t ui_status_menu_init(lv_obj_t *parent) {
    ...
    // Battery monitor rides the same board I2C bus as the touch controller.
    // Only the CrowPanel Advanced 9" carries the STC8H companion MCU; on other
    // boards the probe simply fails and the battery UI stays hidden.
    if (s_touch_i2c_bus) {
        battery_mgr_init(s_touch_i2c_bus);
    }
    ...
    // Battery indicator at the very top, then the remaining status icons stacked
    // directly below it.
    ...
    lv_obj_align(s_icon_batt, LV_ALIGN_TOP_MID, 0, 8);
    ...
    lv_obj_align(s_icon_wifi, LV_ALIGN_TOP_MID, 0, 30);
    ...
    lv_obj_align(s_icon_tailscale, LV_ALIGN_TOP_MID, 0, 52);
    ...
    lv_obj_align(s_icon_ble, LV_ALIGN_TOP_MID, 0, 74);
    ...
    // Battery sits at the very top of the expanded panel, above the terminal
    // text-color slider control (which starts at y=34).
    ...
    lv_obj_align(s_status_batt, LV_ALIGN_TOP_LEFT, 0, 8);

    // WiFi is a two-line label (signal on line 1, SSID on line 2), so the
    // items below it are shifted down to leave room for the extra line.
    ...
    lv_obj_align(s_status_tailscale, LV_ALIGN_TOP_LEFT, 0, 166);
    ...
    lv_obj_align(s_status_ble, LV_ALIGN_TOP_LEFT, 0, 190);
    ...
    if (waveshare_display_brightness_supported()) {
        ...
        // LV_EVENT_ALL so the callback receives both VALUE_CHANGED (live apply)
        // and RELEASED (persist to NVS); the callback filters the rest.
        lv_obj_add_event_cb(s_brightness_slider, brightness_slider_event_cb, LV_EVENT_ALL, NULL);
...
void ui_status_menu_update(void) {
    ...
    // Two-line WiFi status: line 1 shows the signal (RSSI), line 2 shows the
    // SSID (or "disconnected"). Color still conveys connection state.
    if (wifi_connected) {
        int rssi = 0;
        char ssid_line[STATUS_LINE_MAX + 1];
        snprintf(ssid_line, sizeof(ssid_line), "%s", wifi_mgr_get_ssid());
        clamp_line(ssid_line);
        if (wifi_mgr_get_rssi(&rssi)) {
            snprintf(line, sizeof(line), "WiFi: (%d dBm)\n%s", rssi, ssid_line);
        } else {
    ...
    // Battery: read the STC8H companion MCU (CrowPanel Advanced 9" only).
    battery_status_t batt = {};
    const bool batt_ok = battery_mgr_available() &&
                         battery_mgr_read(&batt) == ESP_OK && batt.valid;
    if (batt_ok) {
        const bool charging = battery_mgr_state_is_charging(batt.state);
        lv_obj_clear_flag(s_icon_batt, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(s_status_batt, LV_OBJ_FLAG_HIDDEN);

        if (charging) {
            // Charging: green battery body with a white lightning bolt.
            lv_label_set_text(s_icon_batt, LV_SYMBOL_CHARGE);
            lv_obj_set_style_text_color(s_icon_batt, lv_color_hex(0xFFFFFF), 0);
            lv_label_set_text(s_status_batt, "Battery: charging");
            lv_obj_set_style_text_color(s_status_batt, lv_color_hex(0x2ECC71), 0);
        } else {
            const lv_color_t color = battery_level_color(batt.level_percent);
            lv_label_set_text(s_icon_batt, battery_level_glyph(batt.level_percent));
            lv_obj_set_style_text_color(s_icon_batt, color, 0);
            snprintf(line, sizeof(line), "Battery: %u%%", (unsigned)batt.level_percent);
            lv_label_set_text(s_status_batt, line);
            lv_obj_set_style_text_color(s_status_batt, color, 0);
        }
    } else {
        lv_obj_add_flag(s_icon_batt, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(s_status_batt, LV_OBJ_FLAG_HIDDEN);
