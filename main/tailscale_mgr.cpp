...
// Tracks whether the last periodic tick saw Tailscale enabled, so we log exactly
// once on the enabled->disabled transition and then stay quiet while disabled.
static bool s_periodic_logged_enabled = true;
...
void tailscale_mgr_process_queue(void) {
    ...
    // Only emit the periodic status log while Tailscale is enabled. When it is
    // disabled there is nothing to report and the 15s log just spams the console
    // (and looks like it correlates with unrelated async errors). Still log once
    // on the enabled->disabled transition so the state change is visible.
    const int64_t now_us = esp_timer_get_time();
    if (s_cfg.enabled) {
    ...
    } else if (s_periodic_logged_enabled) {
        ts_log_status("disabled");
        s_last_status_log_us = now_us;
    }
    s_periodic_logged_enabled = s_cfg.enabled;
}
