...
extern "C" esp_err_t nvs_set_u32(nvs_handle_t handle, const char *key, uint32_t value) {
    ...
    NvsEntry e;
    e.data.assign(4, 0);
    for (int i = 0; i < 4; i++) e.data[i] = (uint8_t)((value >> (8 * i)) & 0xFF);
    g_store[h->ns][key] = e;
    return ESP_OK;
}
...
extern "C" esp_err_t nvs_get_u32(nvs_handle_t handle, const char *key, uint32_t *out_value) {
    ...
    if (ns == g_store.end()) return ESP_ERR_NVS_NOT_FOUND;
    auto it = ns->second.find(key);
    if (it == ns->second.end() || it->second.data.size() < 4) return ESP_ERR_NVS_NOT_FOUND;
    uint32_t v = 0;
    for (int i = 0; i < 4; i++) v |= ((uint32_t)it->second.data[i]) << (8 * i);
    *out_value = v;
    return ESP_OK;
}
...
namespace {
esp_err_t s_i2c_probe_result = ESP_OK;
esp_err_t s_i2c_xfer_result = ESP_OK;
uint8_t s_i2c_regs[256] = {0};
int s_i2c_dev_marker = 0;  // address used as the opaque device handle
}  // namespace

extern "C" void i2c_mock_reset(void) {
    s_i2c_probe_result = ESP_OK;
    s_i2c_xfer_result = ESP_OK;
    memset(s_i2c_regs, 0, sizeof(s_i2c_regs));
}

extern "C" void i2c_mock_set_probe_result(esp_err_t result) {
    s_i2c_probe_result = result;
}

extern "C" void i2c_mock_set_reg(uint8_t reg, uint8_t val) {
    s_i2c_regs[reg] = val;
}

extern "C" void i2c_mock_set_transfer_result(esp_err_t result) {
    s_i2c_xfer_result = result;
}

extern "C" esp_err_t i2c_master_probe(i2c_master_bus_handle_t, uint16_t, int) {
    return s_i2c_probe_result;
}

extern "C" esp_err_t i2c_master_bus_add_device(i2c_master_bus_handle_t,
                                               const i2c_device_config_t *,
                                               i2c_master_dev_handle_t *ret) {
    if (ret) *ret = &s_i2c_dev_marker;  // non-NULL opaque handle
    return ESP_OK;
}
...
/* Emulates the STC8H single-byte register read: write_buffer[0] is the register
 * address, read one byte back from the canned register table. */
extern "C" esp_err_t i2c_master_transmit_receive(i2c_master_dev_handle_t,
                                                 const uint8_t *write_buffer,
                                                 size_t write_size,
                                                 uint8_t *read_buffer,
                                                 size_t read_size, int) {
    if (s_i2c_xfer_result != ESP_OK) return s_i2c_xfer_result;
    if (!write_buffer || write_size < 1 || !read_buffer) return ESP_FAIL;
    uint8_t reg = write_buffer[0];
    for (size_t i = 0; i < read_size; i++) {
        read_buffer[i] = s_i2c_regs[(uint8_t)(reg + i)];
    }
    return ESP_OK;
}
