# DESPOT - Technical Specification

## Overview

DESPOT is an ESP32-P4 touch LCD terminal platform. The primary/default board is the Elecrow CrowPanel Advanced 9" (1024x600 IPS, Wi-Fi 6); the Waveshare ESP32-P4-WIFI6-Touch-LCD-7B is a secondary supported board (legacy path on ESP32-S3). This project is a new take on an old dumb terminal console. It combines a color terminal interface with a simple shell and SSH client so your terminal experience can go with you anywhere.  
...
- Primary/default board: CrowPanel Advanced 9" (ESP32-P4). Secondary board:
  Waveshare ESP32-P4-WIFI6-Touch-LCD-7B (built via the Waveshare overlay).
...
- Wired USB keyboard input is NOT supported on the CrowPanel Advanced 9". The
  board's USB2.0 Type-C port (J16) is wired as a USB device port (VBUS input
  only, diode-fed into the 5V rail; no VBUS output switch, no host `DRVVBUS`
  routing), so it cannot power or enumerate a downstream keyboard. Chip
  revision is not the cause (rev 1.x supports USB host); the limitation is the
  board's port wiring. See "USB HID Host" section below.
...
console_base.cpp
  ...
  -> display backend (crowpanel_display_p4 primary / waveshare_display[_p4]) (+ ch422g on Waveshare)
...
### USB HID Host (`main/usb_hid_host.cpp`, `main/usb_hid_host.hpp`) — NON-FUNCTIONAL on CrowPanel Advanced 9"

- The module still compiles into the P4 build (USB host install, HID boot-keyboard
  parsing, keymap shared with the BLE path), but wired USB keyboards do not work
  on the CrowPanel Advanced 9" for a hardware reason.
- Board hardware analysis (Elecrow schematic V1.2):
  - J16 (USB2.0 Type-C) D+/D- route to ESP32-P4 pads 49/50 (dedicated USB
    analog pins), which belong to USB-DWC controller [0] (High-Speed/UTMI PHY).
    The library default `peripheral_map == 0` correctly selects this controller.
  - `+VBUS2` (J16 VBUS) is Schottky-diode-fed *into* `VCC5V`/`VCC5V_IN`
    (D007/D008). VBUS on this port is an INPUT: the port is designed to receive
    5V from an upstream host, not to source it. There is no VBUS output load
    switch and no host `DRVVBUS` net.
  - Consequently the port cannot power a directly connected keyboard, and even
    with a self-powered hub it does not operate as a host bus.
  - Elecrow's own USB example (`Lesson06-USB2.0/bsp_usb.c`) drives this port in
    TinyUSB *device* mode (the panel enumerates as a HID mouse to a PC),
    confirming J16 is a device port by design.
- Chip revision is NOT the cause. This unit is ESP32-P4 rev 1.x; rev < 3 handles
  the host-mode D+/D- 15k pulldowns in hardware and fully supports USB host. The
  only rev-gated USB code (`CONFIG_ESP32P4_SELECTS_REV_LESS_V3`) concerns pulldown
  ownership on rev >= 3 and is correctly configured for this silicon.
- Resolution: use a BLE keyboard. Enabling wired USB host would require a board
  hardware modification (a VBUS output switch on J16 and host ID/VBUS handling).
...
## Battery, C6 Firmware, BLE, and SDIO Work (2026-07)

This cycle added the battery indicator, upgraded the ESP32-C6 co-processor
firmware, fixed BLE keyboard bring-up on the P4, characterized the SDIO link,
and fixed brightness persistence. Detailed record so the reasoning is not lost.

### 5) Battery indicator (implemented)

- Hardware: the CrowPanel Advanced 9" has an STC8H1Kxx companion MCU on the
  shared board I2C bus (addr `0x2F`, same SDA45/SCL46 bus as the GT911 touch).
  It performs battery ADC sampling + charge management and exposes a register
  block at `0x00` (`Battery_info_t`: `u32 adc_voltage; u32 bat_voltage;
  u8 bat_level; u8 bat_state; u8 led_state;`), matching Elecrow's factory
  `bsp_stc8h1kxx.*`. No extra hardware needed.
- Driver: `main/battery_mgr.{cpp,hpp}`. Registers the STC8H on the existing
  touch I2C bus, probes for ack, and polls once per second in a dedicated
  low-priority task, caching a mutex-guarded snapshot. IMPORTANT: the STC8H does
  NOT support I2C auto-increment block reads — each register byte must be read
  individually (`reg = 0x00 + i`), exactly as the factory driver does. A single
  `transmit_receive` block read returns garbage (was seen as the icon rotating
  between charging/100%/0%). The poll runs off the main loop so it never blocks
  the BLE/SSH pump.
- UI (`ui_status_menu.cpp`): collapsed strip shows a battery glyph at the very
  top (white `LV_SYMBOL_CHARGE` bolt when charging; level glyph colored green /
  yellow (<30%) / red (<=10%) when on battery). Expanded panel shows a two-line
  WiFi entry and a `Battery:` line. `bat_state == 1` means charging.

### 6) ESP32-C6 slave firmware upgrade (2.3.0 -> 2.12.9)

- The board shipped with an Elecrow-internal C6 slave build reporting `2.3.0`
  (NOT a public esp_hosted release; the registry has no 2.3.x). Host esp_hosted
  is `2.12.9`; the mismatch produced `Version mismatch: Host [2.12.0] >
  Co-proc [2.3.0]` warnings and was suspected in transport instability.
- Upgraded the C6 via ESP-Hosted's `host_performs_slave_ota` example (OTA over
  SDIO). The slave (`network_adapter.bin`) was built from
  `managed_components/espressif__esp_hosted/slave` (`idf.py set-target
  esp32c6`). On this v6.1-dev IDF the slave fails to compile unless
  `H_OTA_CHECK_IMAGE_VALIDITY` is forced to 0 in `slave/main/slave_config.h`
  (the `esp_ota_check_image_validity` API is absent in this IDF snapshot).
- The OTA tool must talk to the C6 with the CrowPanel transport profile. The
  proven-working OTA config (from Elecrow's shipped upgrade tool) is 1-bit SDIO,
  reset GPIO 32; the Partition OTA method is more robust than LittleFS (the
  regenerated LittleFS image mounted empty). Flash the app and the slave_fw
  partition in two explicit esptool passes (app `--after no-reset`, then the
  partition `--force`) to avoid the example's flash-step reset collision.
- The C6's original 2.3.0 is UNRECOVERABLE (Elecrow-internal, not shipped as a
  standalone binary, no public source). Do not plan to revert to it.

### 7) BLE keyboard bring-up on ESP-Hosted (root cause + fix)

- After the C6 upgrade, BLE keyboards stopped working: every NimBLE HCI command
  timed out (`BLE_HS_ETIMEOUT_HCI`, "HCI wait for ack returned 19").
- Root cause: DESPOT went straight to `nimble_port_init()` without first
  bringing up the BLE controller on the C6. The controller lives on the C6 and
  must be explicitly started over ESP-Hosted. Fix (`ble_hid_host_init`):
  call `esp_hosted_bt_controller_init()` then `esp_hosted_bt_controller_enable()`
  before `nimble_port_init()` (guarded by `CONFIG_IDF_TARGET_ESP32P4`,
  `#include "esp_hosted.h"`). Confirmed working.
- Note: after a P4-only reset (see SDIO section) these can return
  `ESP_ERR_INVALID_STATE`/`ESP_FAIL` because the C6 controller is still up from
  before; that path is benign when the C6 is already enabled.

### 8) SSH "timeout" was the router, not firmware

- Extended chase of SSH `connect() errno=116` (TCP timeout, DNS worked) led
  through several dead ends: BLE/WiFi coexistence relaxation, full BLE
  disconnect during connect, esp_hosted queue-size/mempool changes. NONE fixed
  it, and the mempool change (`USE_MEMPOOL=y` + queues 20) caused a boot crash
  (`esp_startup_start_app` assert, heap exhaustion) and was reverted.
- Actual root cause: the LAN router / DHCP server had crashed (could not resolve
  local DNS names for any device). A router restart fixed SSH immediately.
- All SSH-debug changes were reverted. P4 coexistence remains a no-op stub
  (`coex_manager_stub.cpp`) — the C6 radio's coex APIs are not reachable from
  the host, and coexistence was not the problem.

### 9) SDIO link width / clock (current: 1-bit @ 40 MHz)

- CrowPanel V1.1+ wires the C6 over 4-wire SDIO (D0-D3 = IO17/16/15/14), reset
  GPIO 54. At 4-bit/40 MHz the link showed intermittent, self-recovering SDIO
  read glitches: `sdmmc_send_cmd returned 0x109` / `sdio_get_tx_buffer_num:
  err: 265` (`ESP_ERR_NOT_FOUND`) from a single register read that esp_hosted
  retries and recovers from. Functionally harmless but noisy; WiFi stays up.
- Switched to 1-bit @ 40 MHz (`CONFIG_ESP_HOSTED_SDIO_1_BIT_BUS=y` /
  `_BUS_WIDTH=1` in `sdkconfig.defaults.esp32p4`) which stabilized the link.
  1-bit trades WiFi throughput for signal-integrity margin.
- A tailscale `periodic` log line appeared to precede the 0x109 errors — this
  was coincidental (it was the only line logging every 15s). Unrelated; the
  periodic log is now silenced while Tailscale is disabled.

### 10) Serial-monitor-attach boot loop (CrowPanel hardware quirk, open)

- Attaching the serial monitor pulses the CrowPanel USB-serial (WCH CH343,
  VID 1a86) DTR/RTS, which triggers a P4-only `SW_CPU_RESET` (reset_reason
  decodes as SW). The P4 restarts but the C6 keeps running / goes stale, so the
  next SDIO register read fails (`0x107`). With
  `ESP_HOSTED_TRANSPORT_RESTART_ON_FAILURE` enabled this loops via
  `_h_restart_host()`; disabling it reduces but does not fully eliminate the
  loop on this board.
- Confirmed a HARDWARE difference: the Waveshare P4 board (same P4+C6+esp_hosted)
  gets a clean `POWERON` (both chips reset together) from the identical monitor
  action and never loops. The CrowPanel's reset wiring only resets the P4 CPU.
- Practical workaround: power-cycle the device AFTER the monitor connects, or
  attach with a no-reset monitor (DTR/RTS not toggled). Not firmware-fixable;
  revisit later. `RESTART_ON_FAILURE` is currently DISABLED.

### 11) Brightness persistence fix

- Display backlight brightness was never saved to NVS and never loaded at boot,
  so it always powered up at the backend's hardcoded 25% default. All other
  user settings (termfg color, idle timeout, hostname, wifi, ssh, tailscale,
  vault, BLE pairing) already persist correctly.
- Fix: added `brightness_save()`/`brightness_load()` in `shell.cpp` (NVS
  `devicecfg`/`blpct`, u8 0-100), mirroring the `idleTimeout` pattern. The
  brightness slider persists on `LV_EVENT_RELEASED` (not every drag step, to
  spare flash), and `brightness_load()` is called at boot in `console_base.cpp`
  alongside `idle_timeout_load()`. Keep NVS persistence in `shell.cpp`.
...
> Applies to the secondary Waveshare ESP32-P4-WIFI6-Touch-LCD-7B board only
> (its specific charger/power-path topology). The primary CrowPanel Advanced
> 9" board has a different power path and uses the default brownout level.
